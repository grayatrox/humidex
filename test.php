...
$page          = "./test.php"; // The location of this file in relation to domain
$sec           = "10"; //how often AJAX refreshes the data in minutes
$API           = 'c6f51af1552ce48878c7e05222816ca2'; // This is the api from openweather map
...
function haversineGreatCircleDistance($latitudeFrom, $latitudeTo, $longitudeFrom, $longitudeTo, $earthRadius = 6371000){
...
function getNearestStation($location){
	global $stations;
	$min_val = null;
	$min_key = null;
	foreach ($stations as $key => $value) {
		if (isset($location['city']) && strtolower($location['city']) == strtolower($stations[$key]['name'])){
			return $stations[$key];
		} else {
			$newStations[] = array(
...
$distance = haversineGreatCircleDistance($center_lat, $lat, $center_lng, $lng, 6371/100);
//print('The test distance is : '.$distance.'<br>');
if (isset($postData['auto_loc']) || isset($postData['manual_loc'])) {
    ...
    try {
		$response = getData('http://bom.gov.au/fwo/' . $station['json']);
...
?><html>
  ...
  <body>
    <div id='container'>
        <div id='body' style='margin:1em'>
            <div class='frm_location' style='visibility:visible' >
                <form onsubmit='return verifyInput();'>
                    <input id='frm_loc' type='text' onfocus='frm_onFocus(this)' onfocousout='frm_focusOut(this)' style='width:70%;float:left' placeholder='Suburb or postcode'/>
                    <input class='enableOnInput' type='submit' disabled='disabled' value='Submit'style='width:30%;float:left'>
                </form>
            </div>
        	...
        	<div id='weatherResults'  style='visibility: hidden;position: relative;'>
            ...
            <div >
                <ul style='list-style-type: none;margin-right:1em;'>
                    ...
                    <li style='padding-top:1em'>UV Index: <span id='uvIndex'></span>(placeholder - not currently updated.)</li>
